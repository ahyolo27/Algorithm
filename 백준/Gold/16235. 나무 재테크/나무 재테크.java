...
public class Main {
    ...
    static void input() throws IOException {
        ...
        trees = new ArrayDeque[N + 1][N + 1]; // 좌표 별 나무 정보

        for (int i = 1; i <= N; i++) {
            for (int j = 1; j <= N; j++)
                trees[i][j] = new ArrayDeque<>(); // 양분 정보 초기화
    ...
    static void simulate() {
        ...
        for (int i = 1; i <= N; i++) {
            for (int j = 1; j <= N; j++) {
                ...
                Deque<Integer> remains = new ArrayDeque<>(size);
                boolean isFinished = false;
                int deadSum = 0;

                for (int k = 0; k < size; k++) {
                    int tree = trees[i][j].pollFirst();
                    if (!isFinished && map[i][j] >= tree) { // 양분이 충분한 경우
                        map[i][j] -= tree;
                        remains.addLast(tree + 1);
                    } else {
                        isFinished = true;
                        deadSum += tree / 2;
                    }
                }
                trees[i][j] = remains; // 살아남은 나무들 다음 계절로 보내기
